 ...
 *   Accepts remote connections from LangChain/Claude agents via Streamable HTTP transport.
 */
...
async function marketoRequest(
  ...
  const baseUrl = isAsset ? `${MARKETO_BASE_URL}/rest/asset/v1` : `${MARKETO_BASE_URL}/rest/v1`;
  ...
  try {
    ...
    console.error(`[Marketo] fetch error for ${method} ${url.toString()}: ${msg}`);
    return { success: false, errors: [{ code: "FETCH_ERROR", message: `${method} ${endpoint} — ${msg}` }] };
  }
...
function registerLeadDatabaseTools(server: McpServer): void {
  ...
  server.tool(
    ...
    async ({ listId, leadIds }) => {
      ...
      return toolResult(await marketoRequest("DELETE", `/lists/${listId}/leads.json`, undefined, params));
    }
  );
...
async function main() {
  ...
  if (hasTLS) {
    ...
    httpsServer.listen(HTTPS_PORT, "0.0.0.0", () => {
      ...
      console.log(`  Streamable HTTP: https://agent.tmrwgroup.ai:${HTTPS_PORT}/mcp`);
    });
  } else {
